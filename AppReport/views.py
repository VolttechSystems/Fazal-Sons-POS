...
# )
    cursor = connections['default'].cursor()
    transaction = "select created_at::date, sum(grand_total::INTEGER) from tbl_transaction where grand_total > 0 and outlet_code_id = '"+ outlet +"' and created_at::date BETWEEN '"+ str(parsed_start_date) +"' and '" + str(parsed_end_date) +"'  GROUP BY created_at::date"
